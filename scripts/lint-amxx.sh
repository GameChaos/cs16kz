...
#
# See kz_api_get_map_details in src/kz_natives.cpp.
set -euo pipefail
...
if rg --pcre2 -q "${PATTERN}" "${ROOT}/src/"; then
  echo "::error::MF_GetAmxString misuse — returns AMXX-managed char*; do not pass a local buffer or sizeof (see kz_api_get_map_details in src/kz_natives.cpp)"
