...
void kz_ws_run_tasks(int max_tasks_per_frame)
{
    ...
    while(g_incoming_queue.front() && tasks_done < max_tasks_per_frame)
    {
        ...
        {
            g_callback_table[index](json_object_get_object(root_obj, "data"));
        }
...
{
    MF_Log("[kz_ws_ack_invalid] Invalid msg_id: %d", json_object_get_number(obj, "msg_id"));
}
void kz_ws_ack_hello(JSON_Object* obj)
{
    int heartbeat_interval = json_object_get_number(obj, "heartbeat_interval");
    
    g_websocket.setPingInterval(heartbeat_interval);
    g_websocket.setPingTimeout(10);
...
void kz_ws_ack_map_info(JSON_Object* obj)
{
    ...
    const char* length = json_object_get_string(obj, "length");
    const char* difficulty = json_object_get_string(obj, "difficulty");

    // TODO: plugin-api
    MF_Log("[kz_ws_ack_map_info] -> [type:%s | length: %s | difficulty: %s]", type, length, difficulty);
}
void kz_ws_ack_client_info(JSON_Object* obj)
{
    bool is_banned = json_object_get_boolean(obj, "banned");
    ...
    const char* authid = json_object_get_string(obj, "steamid");
    if(!authid || !authid[0])
    {
        return;
    }
    ...
    char buff[192];
    const char* banned_by = json_object_get_string(obj, "by");
...
void kz_ws_ack_new_record(JSON_Object* obj)
{
    MF_Log("[kz_ws_ack_map_info]");
}