...
uint32_t UTIL_CRC32(const void *data, size_t dataLength)
{
	...
	{
		uint8_t index = static_cast<uint8_t>((crc ^ *cursor) & 0xFF);
		crc = kCRCTable[index] ^ (crc >> 8);
	}

	return ~crc;
}
uint32_t UTIL_CRC32_Incremental(const void *data, size_t dataLength, uint32_t currentCrc)
{
    const uint8_t *cursor = static_cast<const uint8_t*>(data);
...
uint32_t get_map_crc32(const char* mapname)
{
    char path[256];
    MF_BuildPathnameR(path, sizeof(path), "maps/%s.bsp", mapname);
    ...
    {
        crc = UTIL_CRC32_Incremental(buffer, bytes_read, crc);
    }
    fclose(fp);
    return ~crc;
}
void to_base36(uint64_t value, char* dest, size_t len)
{
    char buff[16];
    ...
    *p = '\0';
    ...
    strncpy(dest, p, len - 1);
    dest[len - 1] = '\0';
}
void replace(char* c, char token, char with)
{
    while (*c)
    {
        if (*c == token)
...
size_t remove_substring(char* text, const char* what)
{
    if (!text || !what) return 0;

    size_t textLen = strlen(text);
    size_t whatLen = strlen(what);

    if (whatLen > textLen || whatLen == 0)
        return 0;

    // Search for the substring
    char* match = strstr(text, what);

    if (match)
    {
        // Calculate how many characters are after the match (including \0)
        char* rest_of_string = match + whatLen;
        size_t restLen = strlen(rest_of_string);

        // Move the tail of the string over the 'what' location
        // memmove is safe for overlapping memory
        memmove(match, rest_of_string, restLen + 1);

        return textLen - whatLen;
    }
...
edict_t* find_player_by_authid(const char* authid)
{
