...
{
	return MF_PrepareCellArrayA(reinterpret_cast<cell *>(&vec), 3, true);
}
inline cell privateToIndex(const void* pdata)
{
	if (pdata)
	{
		char* ptr = const_cast<char*>(reinterpret_cast<const char*>(pdata));
		ptr += 4;
...
{
	return *reinterpret_cast<T *>((reinterpret_cast<int *>(ptr) + offset + (element * sizeof(T))));
}
...
{
	return reinterpret_cast<T *>(reinterpret_cast<int *>(ptr) + offset + (element * size));
}
