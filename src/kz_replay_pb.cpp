...
// Replay uid: <class:1>_<time_ms>_<steamid>_<timestamp>.krpz — PRO (0) beats NUB (1), then lowest time.
static bool kz_pb_parse_replay_uid(const std::string& filename, uint8_t& run_class, uint32_t& time_ms)
{
...
static bool kz_pb_replay_is_better(uint8_t cls_a, uint32_t time_a, uint8_t cls_b, uint32_t time_b)
{
    if (cls_a != cls_b)
    {
        return cls_a < cls_b;
...
std::filesystem::path kz_pb_find_fastest(const char* mapname)
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator(map_dir))
    {
        ...
        uint8_t run_class = 0;
        uint32_t time_ms = 0;
        if (!kz_pb_parse_replay_uid(filename, run_class, time_ms))
        {
            continue;
        }

        if (!have_best || kz_pb_replay_is_better(run_class, time_ms, best_class, best_time_ms))
        {
            have_best = true;
